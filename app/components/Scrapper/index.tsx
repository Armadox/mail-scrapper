...
export default function Home() {
  ...
  const exportToExcel = (emails: PlaceWithEmail[]) => {
    ...
    ws["!cols"] = [{ wch: 100 }, { wch: 60 }, { wch: 30 }, { wch: 30 }];
  ...
  const fetchPlaces = async () => {
    ...
    const data: { name: string; address: string; website?: string }[] =
      await response.json();
  ...
  return (
    <div className="max-w-[1000px] flex-1">
      ...
      {noApi && (
        <Alert
          variant="destructive"
          className="my-9 border-red-800 text-red-800"
        >
          <AlertCircle className="h-4 w-4" />
          <AlertTitle>Missing API or Search</AlertTitle>
          <AlertDescription>
            Please insert a valid Google API or a valid String into the search
            input.
          </AlertDescription>
        </Alert>
      )}
      ...
      {websiteCount > 0 && (
        <Progress
          ...
          value={(mailCount / websiteCount) * 100}
        />
      )}
      <form onSubmit={handleSubmit} className="my-5">
        <div className="grid w-full max-w-sm items-center gap-1.5 my-3">
          ...
          <div className="text-xs">
            You can get a Google API from the Google cloud Console.
          </div>
        </div>
        <div className="grid w-full max-w-sm items-center gap-1.5 my-3">
          <Label>Search: </Label>
          ...
          />
          <div className="text-xs">
            Insert something like `&quot;`Bayern Yoga`&quot;`.
          </div>
        </div>
        <Button
          type="submit"
          className="my-3 bg-slate-100 text-gray-950 hover:bg-slate-300 hover:text-black"
          disabled={websiteCount !== mailCount || loading}
        >
